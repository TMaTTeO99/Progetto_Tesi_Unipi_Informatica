...
public class MyTimerTask extends TimerTask {
                                ...
                                public synchronized void onResponse(JSONObject response) {
                                    ...
                                    if(v == null){
                                        ...
                                        if(flag.get(0)) {
                                            ...
                                            //v = database.ChannelDao().findByName(channelID, READ_KEY);
                                        }
                                        ...
                                        /**
                                         * Resetto i flag fra i diversi recuperi del channel dal db
                                         */
                                        //checkFlags.resetFlag(false);
                                    }
                                    try {
    ...
    private void distanza(String data, String channelID) {
        ...
        if(!checkFlags.isFlag_ctime1()){
            text1.setText(toSet);
            checkFlags.setFlag_ctime1(true);
        }
        else {}
    }
     ...
     * Usato per impostare il nome del field
     */
    /*private void setFieldName(ArrayList<String> fields, int field){


        if (!fields.isEmpty() && fields.get(field) != null){

            String name = fields.get(field);
            if(name.equals("P0") || name.equals("windspeed")) {

                //scelgo il field corretto da settare
                select_check_Field(field, 2, name);
            }
            else{

                if(select_check_Field(field, -1, name) == null){
                    select_check_Field(field, 1, fields.get(field));
                }
                else if(select_check_Field(field, -2, name) == null){

                    if(!select_check_Field(field, -1, name).equals(name)){
                        select_check_Field(field, 2, fields.get(field));
                    }
                }
    ...
    /**
     * Metodo aggiunto da Matteo Torchia 599899
     * Metodo usato per settare valori
     */
    /*private String setDataField(int position, int flagPosition, int imagePosition, int positionField, String field, String fieldName, ArrayList<String> fields, JSONObject valori, ArrayList<String> fieldsKey) throws Exception{

        String value = null;

        int tmpPos = retreivePos(position);
        if(tmpPos > 8) {

            int realPoss = ((tmpPos - 1) % 8);

            String fieldSelected = select_check_Field(realPoss, -2, "");
            int idxField = fields.indexOf(fieldSelected);

            //succede che se ho un canale come che è tipo filed1,filed2, filed5, filed7, filed8
            //quindi ha dei buchi, l'indice dell'elemento della lista delle selezioni dell'utente
            //è diverso da dove si trova effettuvamente il field, quindi idxField == -1 solo perche
            //select_check_Field restituisce null in quanto il field si trova
            //in una variabile diverse, ad es: magari dovevo recuperare
            //il field in v.getField2_2 e invece si trova in v.getField2_3 (c'è un buco)
            //quindi è un caso da controllare

            //TODO: testing
            //TODO: provo un altra idea,in caso non vada devo modificare l'inserimento, quando recupero i nomi
            //TODO: recupero le loro chiavi, ad esempio field1,field2, ecc
            //TODO: e poi li "comopatto", li scorro tutti e li inserisco uno alla volta
            ////TODO: nei loro rispettivi field usando select_check_Field.

            if(idxField == -1)idxField = JumpFields(-2, realPoss);


            //caso in cui nel canale corrente il field è presente lo inserisco
            if(idxField != -1) {

                //TODO: testing, da modificare va spsotato il controllo sui valori in un if interno e
                //TODO: prima bisogna fare jumpField
                if(selectImage(imagePosition) != null && !valori.getString(fieldsKey.get(idxField)).equals("") && !valori.getString(fieldsKey.get(idxField)).equals("null")) {
                    value = valori.getString(fieldsKey.get(idxField));
                    selectFlag(flagPosition, 1, true);
                }
                else if (v.getImagetemp()==null && fields.get(positionField).equals(fieldName) && !valori.getString(field).equals("") && !valori.getString(field).equals("null")) {
                    value = valori.getString(field);
                    selectFlag(flagPosition, 1, true);
                }
            }
        }
            ...
            if (selectImage(imagePosition) != null && !valori.getString(selectImage(imagePosition)).equals("") && !valori.getString(selectImage(imagePosition)).equals("null")){

                int realPoss = ((tmpPos - 1) % 8);

                String fieldSelected = select_check_Field(realPoss, -1, "");
                int idxField = fields.indexOf(fieldSelected);

                //TODO: testing
                //succede che se ho un canale come che è tipo filed1,filed2, filed5, filed7, filed8
                //quindi ha dei buchi, l'indice dell'elemento della lista delle selezioni dell'utente
                //è diverso da dove si trova effettuvamente il field


                //TODO: testing
                if(idxField == -1)idxField = JumpFields(-1, realPoss);
            ...
            else if (fields.get(positionField).equals(fieldName) && selectImage(imagePosition)==null && !valori.getString(field).equals("") && !valori.getString(field).equals("null")) {
                value = valori.getString(field);
                selectFlag(flagPosition, 1, true);
            }
        }
    ...
    /*
      private ArrayList<String> retreiveFieldsJump(int choice) {

        ArrayList<String> fields = new ArrayList<>();
        for(int i = 0; i<8; i++){
            String x = select_check_Field(0, choice, "");
            if(x != null)fields.add(x);
        }
        return fields;
    ...
    /**
     * Metodo aggiunto da Matteo Torchia 599899
     * Controllo in caso di piu di 8 field selezionati
     * se sono stati inseriti nelle corrette posizioni
     */
    /*private int JumpFields(int choice, int idxNotFound){

        int i = 1, j = 0;
        String prev = null;
        String curr = select_check_Field(0, choice, "");
        while (i < 8 ){

            prev = curr;
            curr = select_check_Field(i, choice, "");
            ...
            if(prev == null){
                ...
                return i + (idxNotFound - j);
            }
            j++;
            i++;
        }
        return -1;//caso in cui non ci sono buchi
        //[field1],[field2 ],[field3 buco],[field4buco],[field5 buco],[field6 buco],[field7],[field8]
    ...
    /**
     * Metodo aggiunto da Matteo Torchia 599899
     * Usato per selezionare il flag del valore corretto,
     * flag usati per non sovrascrivere i dati
     */
    /*private boolean selectFlag(int position, int choice, boolean value){
        ...
        switch (position) {
            ...
            case 7:
                ...
                if(choice == 0)flag = checkFlags.isFlag_field8();
                else checkFlags.isFlag_field8();
                break;
            case 8:
            ...
            case 10:
                ...
                if(choice == 0) flag = checkFlags.isFlag_EVT();
                else checkFlags.setFlag_EVT(value);
    ...
    /**
     * Metodo aggiunto da Matteo Trochia 599899
     */
    /*private String select_check_Field(int field, int choice, String fieldName) {
     ...
     */
    /*private int retreivePos(int field){
